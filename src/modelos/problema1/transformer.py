# src/modelos/problema1/transformer.py
import os, json
...
CONFIG = {
    'lookback': 24,
    'd_model': 64,       # dimensión de embeddings
    'num_heads': 4,      # cabezas de atención
    'ff_dim': 128,       # dimensión feed-forward interno
    'num_blocks': 2,     # bloques transformer apilados
    'dropout': 0.1,
    'epochs': 100,
    'batch_size': 512,
    'learning_rate': 0.001,
...
def transformer_block(x, d_model, num_heads, ff_dim, dropout):
    """
    Encoder block: Multi-Head Self-Attention + Feed-Forward.
    Cada timestep atiende a todos los demás timesteps de la secuencia.
    """
    ...
    x = layers.LayerNormalization(epsilon=1e-6)(x + attn)  # residual
...
# Filtrar top-N zonas (mismo criterio que lstm.py para comparación justa)
...
joblib.dump(le, MODELS_DIR / 'transformer_zona_encoder.pkl')
print(f"  Zonas: {len(le.classes_)}")

# ── 4. Log1p + normalización ──────────────────────────────────────────────────
print("Aplicando log1p y normalizando...")
for df in [df_train, df_val, df_test]:
...
joblib.dump(scaler_X, MODELS_DIR / 'transformer_scaler_X.pkl')
joblib.dump(scaler_y, MODELS_DIR / 'transformer_scaler_y.pkl')
...
# ── 7. Construir modelo Transformer ──────────────────────────────────────────
print("Construyendo Transformer...")

n_features = len(FEATURES)
d_model    = CONFIG['d_model']

inputs = keras.Input(shape=(LB, n_features))

# Proyección lineal: features → d_model
x = layers.Dense(d_model)(inputs)

# Positional encoding: suma un embedding aprendido por posición temporal
# Cada posición (hora t-24 ... t-1) tiene su propio vector
positions = ops.arange(start=0, stop=LB, step=1)
pos_emb   = layers.Embedding(input_dim=LB, output_dim=d_model)(positions)
x = x + pos_emb  # (batch, LB, d_model)
...
# Pooling: usar solo el último timestep para predecir t+1
# (equivalente a "leer" toda la secuencia y predecir el siguiente)
x = x[:, -1, :]  # (batch, d_model)
...
model = keras.Model(inputs, outputs, name='Transformer_Demanda')
model.compile(
...
# ── 10. Guardar ───────────────────────────────────────────────────────────────
model.save(MODELS_DIR / 'transformer_model.keras')

results = {
    'modelo': 'Transformer',
    'nota': f'Transformer encoder sobre top {TOP_N} zonas. '
            f'Positional encoding aprendido. {CONFIG["num_blocks"]} bloques, '
            f'{CONFIG["num_heads"]} cabezas de atención.',
...
axes[2].plot(y_te_pred[:n],     label='Transformer pred', color='#3498db', lw=1.5, alpha=0.8)
...
plt.suptitle(f'Transformer — Top {TOP_N} zonas (Problema 1)', fontweight='bold')
plt.tight_layout()
plt.savefig(PLOTS_DIR / 'transformer_resultados.png', dpi=150, bbox_inches='tight')
plt.show()

print(f"\nPlot:       {PLOTS_DIR / 'transformer_resultados.png'}")
print(f"Resultados: {RESULTS_DIR / 'transformer_results.json'}")